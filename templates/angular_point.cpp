...
struct angular_point
{
    ...
    auto operator<=>(const angular_point &other) const -> weak_ordering
    {
...
auto main() -> i32
{
    ...
    auto w = vector(v);
    sort(w.begin(), w.end(), greater<angular_point>());
    ...
    while (k--)
    {
        ...
        auto p = v[--i], q = v[--j];
        auto lo = partition_point(w.begin(), w.end(), [&](angular_point r)
                                  { return r > p; });
        auto hi = partition_point(w.begin(), w.end(), [&](angular_point r)
                                  { return r >= q; });

        auto d = static_cast<i64>(hi - lo);
        if (d <= 0)
        {
            d += n;
        }
