...
struct segment
{
    ...
    auto overlaps(segment<T> &other) const -> bool
    {
        return max(l, other.l) < min(r, other.r);
    }

    template <char true_side, i32 iteration = 100, typename F>
        requires is_invocable_r_v<bool, F, T>
    auto bisect(F test) const -> optional<segment>
    {
    ...
    template <i32 iteration = 100, typename F>
        requires is_invocable_r_v<T, F, T>
    auto trisect(F loss) const -> T
    {
        auto l = this->l, r = this->r;
