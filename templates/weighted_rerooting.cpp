...
template <typename S, typename T, auto op, auto add_edge, auto add_vertex, auto e>
    requires is_invocable_r_v<S, decltype(op), S, S> &&
             is_invocable_r_v<S, decltype(add_edge), S, T> &&
             is_invocable_r_v<S, decltype(add_vertex), S, i32> &&
             is_invocable_r_v<S, decltype(e)>
struct rerooting
{
    explicit rerooting(i32 n) : graph(n), dp(n), results(n, e()) {}
