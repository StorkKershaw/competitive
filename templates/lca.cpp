...
{
    explicit lowest_common_ancestor(i32 n) : built(false), n(n), h(1), graph(n), distances(n)
    {
        while ((1 << h) < n)
        {
            ++h;
        }
        parents.resize(h, vector(n, -1));
    }
    ...
    auto query(i32 u, i32 v) const -> i32
    {
        assert(built);
    ...
    auto distance(i32 u, i32 v) const -> i32
    {
