...
template <typename T, i32 D = 60>
    requires (is_integral_v<T> || is_base_of_v<internal::static_modint_base, T>)
struct xor_sum
{
    explicit xor_sum(vector<i64> &v) : zero(), one()
    {
        for (auto x : v)
        {
            for (auto d = 0; d < D; ++d)
...
private:
    ...
    auto pow2(i64 k) const -> U
    {
        return U(1) << k;
    }

    template <typename U = T>
        requires is_base_of_v<internal::static_modint_base, U>
    auto pow2(i64 k) const -> U
    {
        return U(2).pow(k);
    }
