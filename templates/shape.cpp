...
struct shape
{
    shape(i32 h, i32 w) : cells(h, vector(w, false)), positions() {}
    explicit shape(vector<string> &v) : cells(), positions()
    {
        for (auto &s : v)
        {
    ...
    auto translated(i32 dy, i32 dx) const -> optional<shape<C>>
    {
...
auto movements(shape<C> &s, shape<C> &x) -> vector<shape<C>>
{
    ...
    {
        if (auto t = s.translated(p - i, q - j); t && x.contains(t.value()))
        {
            v.push_back(t.value());
        }
    }
