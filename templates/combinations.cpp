...
template <typename Action, typename Yield>
    requires invocable<Action, i32, i32> &&
             invocable<Yield, vector<i32> &>
auto combinations(i32 n, i32 k, Action action, Yield yield) -> void
{
    auto v = vector(k, 0);
...
auto main() -> i32
{
    ...
    auto m = abs(stoll(string(s.size() - 1, '9')) - stoll(s));

    auto nop = [](i32, i32) {};
    auto yield = [&](vector<i32> &v)
    {
        auto t = ""s;
        for (auto x : v)
        {
